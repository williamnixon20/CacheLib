...
class OGBinaryReplayGenerator : public ReplayGeneratorBase {
 public:
  ...
  const ColumnTable columnTable_ = {
      ...
      {SampleFields::NEXT_ACCESS_VTIME,
       true,
       {"next_access_vtime"}}, /* required
                                */
  };
 ...
 private:
  ...
  static constexpr size_t maxSlabSize =
      1ULL << facebook::cachelib::Slab::kNumSlabBits;
...
inline std::unique_ptr<OGReqWrapper> OGBinaryReplayGenerator::getReqInternal() {
  ...
  do {
    ...
    if (!parseRequest(line, reqWrapper)) {
      ...
      size_t totalSize = std::accumulate(reqWrapper->sizes_.begin(),
                                         reqWrapper->sizes_.end(), size_t(0));
...
inline std::unique_ptr<OGReqWrapper>
OGBinaryReplayGenerator::getReqInternalZstd() {
  auto reqWrapper = std::make_unique<OGReqWrapper>();
