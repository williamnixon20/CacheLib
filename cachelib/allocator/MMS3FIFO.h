 ...
 public:
  ...
  struct Container {
   ...
   private:
    ...
    void reinsertMain() const noexcept;
...
bool MMS3FIFO::Container<T, HookPtr>::recordAccess(T& node,
  ...
  // No lock needed — flag ops are atomic, time write is safe on x86.
  if (!isAccessed(node)) {
    markAccessed(node);
...
void MMS3FIFO::Container<T, HookPtr>::lazyPromoteSmallTailLocked()
  ...
  // Per S3-FIFO paper: only process Small when it exceeds target size.
...
template <typename T, MMS3FIFO::Hook<T> T::* HookPtr>
void MMS3FIFO::Container<T, HookPtr>::reinsertMain() const noexcept {
  auto& mainList = lru_.getList(LruType::Main);
  auto tail = mainList.getTail();
  while (tail && isAccessed(*tail)) {
    // Accessed: reinsert to head
    mainList.remove(*tail);
    mainList.linkAtHead(*tail);
    unmarkAccessed(*tail);
    tail = mainList.getTail();
  }
}
...
MMS3FIFO::Container<T, HookPtr>::getEvictionAgeStatLocked(
  ...
  // Report Main stats in warmQueueStat
  {
  ...
  // Report Small stats in coldQueueStat
  {
    auto& list = lru_.getList(LruType::Small);
    auto it = list.rbegin();
...
MMContainerStat MMS3FIFO::Container<T, HookPtr>::getStats() const noexcept {
  auto stat = lruMutex_->lock_combine([this]() {
    auto* tail =
        lru_.size() == 0 ? nullptr : lru_.getList(LruType::Small).getTail();
    if (!tail) {
